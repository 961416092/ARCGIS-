define(["dojo/_base/declare",
	...
	function (declare, _WidgetBase, on, lang, query, GraphicsLayer, SimpleFillSymbol,
		SimpleLineSymbol, Graphic, Color,Point,Polygon, Polyline ) {
		return declare('GeojsonClass', _WidgetBase, {
			defaults: {
				map: null,
			},
			_layer: null,
			...
			_init: function () {
				...
				this._lineSymbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_DASH, new Color([255, 0, 0]), 3);
				this._fillSymbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID, this._lineSymbol , new Color([255, 0, 0]));
				query("#addlocallayer").on("click", lang.hitch(this, this.selectFile));
			},
			selectFile: function () {
				...
				this._formData.append("file", ofile);//这个是文件，这里只是演示上传了一个文件，如果要上传多个的话将[0]去掉
				$.ajax({
					type: "POST",//方法类型
					...
					processData: false,
					contentType: false,
					url: "http://localhost:55496/api/Values/InitinalGdal",//url
					success: function (result) {
						//	var jsonf = new GeoJsonUtil();
						var jsonparse = JSON.parse(result);
						var json = newthis_1.geojsonToArcGIS(jsonparse);
						var type = jsonparse.features[0].geometry.type;
						type=(type=="Polygon"||type=='MultiPolygon')?"Polygon":((type=="LineString"||type=="MultiLineString")?"LineString":"Point");
						switch (type) {
							case "Polygon":
								var geo = new Polygon();
								for (var i = 0; i < json.length; i++) {
									...
									var g = new Graphic(geo, newthis_1._fillSymbol, json[i].attributes);
									newthis_1._layer.add(g);
								}
								break;
							case "LineString":
								var pyl = new Polyline();
								for (var i = 0; i < json.length; i++) {
									pyl.paths = json[i].geometry.paths;
									pyl.spatialReference.wkid = json[i].geometry.spatialReference.wkid;
									var g = new Graphic(pyl, newthis_1._lineSymbol, json[i].attributes);
									newthis_1._layer.add(g);
								}
								break;
							case "Point":
						...
						newthis_1.defaults.map.addLayer(newthis_1._layer);
					},
					error: function (e) {
						alert(e);
					}
